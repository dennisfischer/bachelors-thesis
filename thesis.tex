\documentclass[a4paper,12pt]{article}

\usepackage{etoolbox}
\usepackage{fancyvrb}
\usepackage[T1]{fontenc}
\usepackage{graphicx}
\usepackage{import}
\usepackage{minted}
\usepackage{caption}
\usepackage{url}

\usepackage[
	% Prevent hyperlinks from getting an ugly border
    hidelinks,
    % Set proper PDF metadata
    pdftex,
    pdfauthor={Dennis Fischer},
    pdftitle={Detecting Process Memory Tampering},
    pdfsubject={TBD},
    pdfkeywords={dll injection, code injection, injection, code cave, nop, nop hopping, memory, tampering, detection, dll}
]{hyperref}
\usepackage{sections/_meta/tumlogo}

% Don't display "References" when rendering the bibliography
\patchcmd{\thebibliography}{\section*{\refname}}{}{}{}

% Use appropriate font size for line numbers in listings
\renewcommand{\theFancyVerbLine}{{\small\arabic{FancyVerbLine}}}

\newcommand{\code}[1]{\texttt{#1}}

\newcommand{\codefile}[1]{
\inputminted[breakanywhere, breaklines,fontsize=\scriptsize, frame=single, mathescape, linenos, numbersep=5pt, numbersep=5pt, xleftmargin=0pt]{c}{SourceCode/ProcessProtectionDriver/ProcessProtectionDriver/\detokenize{#1}}
\captionof{listing}{Code from ProcessProtectionDriver/\detokenize{#1}}
\label{code:\detokenize{#1}}
}


\setlength{\footnotesep}{0.4cm}
\setlength{\skip\footins}{0.5cm}

\def\doctype{Bachelor's Thesis in Informatics}
\def\title{Detecting Process Memory Tampering}
\def\germantitle{Erkennen von Prozess Speichermanipluationen}
\def\author{Dennis Fischer}
\def\date{Februar 15, 2016}


\begin{document}
\include{sections/_meta/cover}
\include{sections/_meta/page-1}
\include{sections/_meta/single-handed-composition}

\newpage
\thispagestyle{empty}
\null

% \include{sections/_meta/abstract}

\newpage
\pagestyle{empty}
\tableofcontents

\newpage
\pagestyle{plain}
\setcounter{page}{1}

% PUT Content here!
\include{sections/introduction/introduction}
\include{sections/background/background}
\include{sections/implementation/implementation}
\include{sections/evaluation/evaluation}
\include{sections/futurework/futurework}

\include{sections/chrome_elf/chrome_elf}
 
% References
\section{References}
\bibliography{references}
\bibliographystyle{plain}
% PUT Appendix here!
\include{sections/appendix/appendix}

\end{document}
